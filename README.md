...
- 🎬 **Custom Morph** — hand-pick specific photos of different people (not just one person's own timeline), in any order, and play them back as your own custom morph sequence, saved as a GIF or MP4.
- 😀 **Face Expressions** — add a hand-tunable expression to a detected face using independent sliders (smile, eyebrows, cheek raise, mouth shapes, nose sneer, eye squint), powered by real face-mesh/iris/expression-detection AI models rather than a guessed effect.
...
- 🖌️ **Face Paint** — upload any transparent-background PNG (a flag, a pattern, a drawn design) and drag, resize, and rotate it right on the photo, then apply it — it warps naturally to follow the real shape of the detected face and picks up the photo's own lighting instead of sitting on top as a flat sticker. Eyes and the mouth stay clear automatically; uploaded designs are saved so you can reuse them.
